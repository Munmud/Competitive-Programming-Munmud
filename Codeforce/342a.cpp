...
int main ()
{
   //freopen("E:/Desktop/Programming/input.txt", "r" , stdin );
    ...
    FORN(i,n)
    {
        ...
        while ( mark[pos]== true ||  v[pos]%v[i]!=0 || v[pos]/v[i]<=1  ){
            pos++;
            if (pos==n) break ;
        } if (pos==n) break ;
        ...
        auto ij = lower_bound( v.begin() , v.end() , v[pos]*2 ) ;
        ll ses = distance(v.begin() , ij)-1 ;
        while (mark[ses]== true ||  v[ses]%v[pos]!=0  || v[ses]/v[pos]<=1 ) {
                ses++;
                if (ses==n) break ;
        }if (ses==n) break ;
