...
void letsGetStarted()
{
    ...
    bool isTrain[n+5][n+5] ;
    bool visitBus[n+5] ;
    bool visitTrain[n+5] ;

    MEM(visitBus , false) ;
    MEM(visitTrain , false) ;
    MEM(isTrain , false) ;
    ...
    while (!q.empty())
    {
        ...
        FORAB(i,1,n){
            if (!visitBus[i] && isTrain[node][i]==false){
                if (i == n){
                    ans1 = MIN(ans1 , dist+1) ;
                    while (!q.empty()) q.pop() ;
                    break ;
                }
                else {
    ...
    while (!q.empty())
    {
        ...
        visitTrain[node] = true;
        
            
        FORAB(i,1,n){
            if (!visitTrain[i] && isTrain[node][i]==true){
                if (i == n){
                    ans2 = MIN(ans2 , dist+1) ;
                     while (!q.empty()) q.pop() ;
                    break ;
                }
                else {
