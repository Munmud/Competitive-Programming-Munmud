...
vector <int> createArray(string patt)
{
    vector<int> lps(patt.size()) ;
    int index = 0 ;
    for (int i = 1 ; i<patt.size() ; )
    {
        if (patt[index] == patt[i]){
            lps[i] = index+1 ;
            i++, index++ ;
...
int main ()
{
   //freopen("E:/Programming/input.txt", "r" , stdin );
    ...
    vector <int> v = createArray(s) ;
    //for (auto i : v) cout << i << " " ;
    if (v[n-1]==0 ) cout << "Just a legend" << endl ;
    else {
        bool found = false ;
        int x  ;
        for(int i = 1 ; i<n-1 ; i++) {
        ...
        if (found && ans.size()) cout << ans << endl ;
